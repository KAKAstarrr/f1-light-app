...
## 📌 v2 更新摘要（2026-08-25）：天气维度入模型

- **特征升级**：19 → 24 个（新增 5 个天气特征：`weather_is_wet` 干湿 / `weather_air_temp` 气温 / `weather_track_temp` 赛道温度 / `weather_max_rainfall` 最大降雨 / `weather_humidity` 湿度），对应 PRD 3.4.2「环境维度」。
- **数据来源**：`scripts/build_weather_dataset.py` 并发拉取 2018-2025 共 173 场正赛实际天气（FastF1 weather_data，4 workers，断点续跑，每 20 场落盘），产出 `ml/data/weather_cache.csv`。
- **训练侧**：`feature_engineering.py` 新增 `add_weather_features()`，按 (year, round) 场次级合并，缺失用中性值（干地/20°C/30°C/0mm/60%）兜底；`train_xgboost.py` 输出 `xgb_v2.json`。
- **推理侧**：`prediction_service.py` 在线调用 `fetch_fastf1_weather` 采集天气（未来分站/失败 → 中性值）；`model_version` 按特征列数动态判定（≥24 → xgb_v2）；重训前自动回退 xgb_v1.json，再降级 rule_v1。
- **前端**：预测页新增天气摘要栏（干湿/气温/赛道温度/降雨/湿度，中性值时显示「天气未收录」提示）+ 特征按 7 个维度分组折叠展示（排位与发车/车手赛季状态/近5场/赛道历史/车队趋势/赛季上下文/环境与天气），SHAP top-3 与特征重要性均用中文标签。
- **面试可讲点**：① 场次级特征（同场所有车手相同）如何避免训练/推理错位——按 (year, round) 合并而不是按车手合并；② 天气对未来分站不可知 → 中性值兜底 + `is_neutral` 前端降级展示；③ 模型版本动态判定避免硬编码。

**⚠️ 本文档其余章节为 v1（19 特征）实现细节，作为知识点基础保留；与 xgb_v2 冲突处以本摘要为准。**
...
| 对比维度 | 规则加权模型 (rule_v1) | XGBoost (xgb_v1) | XGBoost (xgb_v2，天气版) |
|---------|----------------------|-------------------|--------------------------|
| 权重来源 | 人工设定（35%/25%/15%/15%/10%） | 从历史数据自动学习 | 同 v1，数据更多 |
| 特征数 | 5 | 19（6 组） | 24（7 组，含环境与天气） |
| 特征交互 | 线性加权，不捕获特征间交互 | 树模型天然捕获非线性交互 | 同 v1，可学习天气×位次交互 |
| 可扩展性 | 每加一个特征要手工调权重 | 加特征后自动学习权重 | 加特征后自动学习权重 |
| 可解释性 | 极高（权重一目了然） | 中等（需要 SHAP 辅助解释） | 同 v1 + 前端按维度分组展示 |
| 数据依赖 | 不需要训练数据 | 需要历史数据训练 | 需要历史数据 + 天气缓存 |
| 推理速度 | <1ms | <10ms（可接受） | <10ms + 天气采集耗时 |
